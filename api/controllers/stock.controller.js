...
export const UpdateStock= async (req, res) => {
    ...
    try {
      ...
      selectedProduct.forEach(item => {
        ...
        const { buyingPrice, sellingPrice } = stock[name];
        stock[name].profit += (sellingPrice - buyingPrice) * quantity; // Update profit
      });
  ...
  export const Individual= async (req, res) => {
    ...
    try {
      const stock = await Stock.findById("6659a5511964f3a7ba5c61ac"); // Adjust the ID based on your database
  
      if (stock && stock[chemical] !== undefined) {
        stock[chemical].quantity += parseFloat(quantity);
        const { buyingPrice, sellingPrice } = stock[chemical];
        stock[chemical].profit += (sellingPrice - buyingPrice) * parseFloat(quantity); // Update profit
        await stock.save();
        res.status(200).json({ message: 'Stock updated successfully', stock });
      } else {
        res.status(400).json({ message: 'Invalid chemical name' });
